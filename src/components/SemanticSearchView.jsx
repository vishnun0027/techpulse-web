...
export default function SemanticSearchView({ session }) {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      if (embedError) throw new Error('Neural search error. Please try again later.');
  ...
  return (
    <div className="search-view animate-in fade-in duration-1000" style={{ maxWidth: '800px', margin: '0 auto', padding: '1rem' }}>
      <header style={{ marginBottom: '4rem', textAlign: 'center' }}>
        ...
        <h1 style={{ fontSize: 'clamp(2rem, 5vw, 3rem)', fontWeight: 900, marginBottom: '1rem', lineHeight: 1.1 }}>Ask TechPulse</h1>
        <p style={{ color: 'var(--text-secondary)', fontSize: '1.1rem' }}>Query your intelligence archive by meaning and context.</p>
      </header>

      <form onSubmit={handleSearch} className="search-form-container" style={{ marginBottom: '5rem' }}>
        <div className="search-input-wrapper" style={{ position: 'relative', display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
          <div style={{ position: 'relative', flex: 1, minWidth: '280px' }}>
            <input
              type="text"
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              placeholder="e.g., 'Transformer risk mitigation'..."
              style={{ 
                width: '100%', 
      ...
      <div style={{ display: 'grid', gap: '1.5rem' }}>
        {results.length > 0 ? (
          results.map((art) => (
            <div key={art.id} className="glass-panel hoverable" style={{ padding: 'clamp(1.25rem, 4vw, 2rem)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem', gap: '1rem' }}>
                ...
                <a href={art.source_url} target="_blank" rel="noreferrer" style={{ color: 'var(--text-muted)', transition: 'color 0.2s', flexShrink: 0 }} onMouseOver={e => e.currentTarget.style.color='white'} onMouseOut={e => e.currentTarget.style.color='var(--text-muted)'}>
                  <ExternalLink size={18} />
                </a>
              </div>
              <h3 style={{ fontSize: 'clamp(1.25rem, 3vw, 1.5rem)', fontWeight: 800, marginBottom: '0.75rem', lineHeight: 1.3 }}>{art.title}</h3>
